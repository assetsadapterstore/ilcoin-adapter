...
func (wm *WalletManager) getBlockByExplorer(hash string) (*Block, error) {
	...
	block := newBlockByExplorer(result)

	miniblocks := result.Get("miniblocks").Array()

	for _, mb := range miniblocks {
		path = "miniblock/"+mb.String()
		result, err = wm.ExplorerClient.Call(path, nil, "GET")
		if err != nil {
			return nil, err
		}
		txs := result.Get("tx").Array()
		for _, tx := range txs {
			block.tx = append(block.tx, tx.String())
		}
	}
	return block, nil
}
